...
nets = dict(
    ...
    litecoin_testnet=math.Object(
     ...
     bonuscoin=math.Object(
        P2P_PREFIX='12345678'.decode('hex'), #OK
        P2P_PORT=65000, #OK
        ADDRESS_VERSION=19, #OK
        RPC_PORT=65001, #OK
        RPC_CHECK=defer.inlineCallbacks(lambda bitcoind: defer.returnValue( #OK
            'bonusaddress' in (yield bitcoind.rpc_help()) and #OK
            not (yield bitcoind.rpc_getinfo())['testnet'] #OK
        )), #OK
        SUBSIDY_FUNC=lambda nBits, height: __import__('bonuscoin_subsidy').GetBlockValue(nBits, height),
        POW_FUNC=data.hash256, #OK
        BLOCK_PERIOD=120, # s #OK
        SYMBOL='BNS', #OK
        ...
        BLOCK_EXPLORER_URL_PREFIX='http://explorer.bonuscoin.net/chains', #OK
        ADDRESS_EXPLORER_URL_PREFIX='http://explorer.bonuscoin.net/chains', #OK
        TX_EXPLORER_URL_PREFIX='http://explorer.bonuscoin.net/chains', #OK
        SANE_TARGET_RANGE=(2**256//2**32 - 1, 2**256//2**32 - 1), #OK
        DUMB_SCRYPT_DIFF=1,
        DUST_THRESHOLD=1e8,
    ),
